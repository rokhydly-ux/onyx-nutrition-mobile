...
export default function MyDayScreen() {
  ...
  const handleRegenerateMenu = async (profileData: any) => {
    ...
    try {
      ...
      const { data: rawRecipes } = await supabase.from('nutrition_recipes').select('*');
      console.log('Recettes reçues:', rawRecipes);
  ...
  const handleRemoveMeal = async (meal: any) => {
    ...
    try {
      ...
      const parsedResults = (offData.products || []).map((p: any) => ({
        ...
        img: p.image_front_url || 'https://res.cloudinary.com/dtr2wtoty/image/upload/v1786107893/Ceramic_plate_with_herbs_on_202608071304_bl72q1.jpg',
        type: '100g'
