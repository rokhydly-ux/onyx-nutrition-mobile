...
import { CheckCircle, Lock, Plus, RefreshCw, ShoppingCart } from 'lucide-react-native';
...
export default function MenuScreen() {
  const { loading, generating, profile, menu, generateMenu, swapMeal, consumedMeals } = useMenuData();
  ...
  // Paywall Logic
  const isPremium = profile?.plan_type === 'premium' || (profile?.daysLeft ?? 0) > 0;
        ...
        {selectedDay?.meals.map((meal) => {
          ...
          return (
            <View key={meal.id} className="bg-white rounded-[24px] overflow-hidden shadow-sm">
              ...
              {/* Actions */}
              <View className="p-4 flex-row gap-3 bg-white">
                <TouchableOpacity
                ...
                <TouchableOpacity
                  className={`flex-1 flex-row justify-center items-center py-3 rounded-[16px] ${
                    isConsumed ? 'bg-[#39FF14]/20' : 'bg-black'
                  }`}
                  disabled={isConsumed}
                ...
                >
                  {isConsumed ? (
                    <>
                      <CheckCircle size={18} color="#39FF14" className="mr-2" />
                      <Text className="text-[#39FF14] font-bold" style={{ fontFamily: 'Poppins_700Bold' }}>Ajouté</Text>
                    </>
                  ) : (
                    <>
